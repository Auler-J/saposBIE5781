...
# 3. Simulação
O código das simulações está em "R/simula_dados.R".
...
Por esse motivo, comparamos o modelo aleatório mais simples possível, apenas com a probabilidade de seleção dos machos. Pelo funcionamento da simulação, a variável \code{prob} deve abarcar toda a informação de \code{freq}, \code{foot} e \code{tama}. No entanto, essa probabilidade não é biologicamente plausível de ser coletada, e usamos aqui apenas para verificar a necessidade do uso do modelo misto.
```{r}
