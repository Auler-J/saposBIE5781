...
<center>Figura 1: Ilustração de rã-de-corredeira (Hylodidae: Hylodes asper).</center>
...
- (I) frequência dominante (DF: medida em Hz), definida como a frequência no espectro harmônico com maior concentração de energia que é percebida pelo ouvido humano como o quão grave ou agudo os sons são (Köhler et al. 2017); 

- (II) comprimento rostro-cloacal (SVL: *snout-vent length*, medida em mm), o qual é amplamente usado como aproximação do tamanho corporal pois mede o comprimento da extremedidade anterior do focinho até a extremidade posterior da cloaca (Duellman 1970); 

- (III) taxa de *foot-flagging* (medida em número de eventos/minuto). 

Para *Hylodes asper*, Haddad & Giaretta (1999) reportaram que a DF se encontra no terceiro harmônico entre 5000 a 6500 Hz (Fig. 3), com valor de *midpoint* igual a 5750 Hz. Heyer et al. (1990) reportaram que os machos apresentam um SVL médio de 40.5 mm (39.4-42.3 mm). Já a taxa de *foot-flagging* não foi reportada por unidade de tempo em nenhum destes trabalhos, mas é possível estimar através de vídeos disponíveis no YouTube (~1-2/min). Utilizamos estes valores como referência das características dos machos simulados na função 'saposim'.
...
# 1. Histograma de tempo até início da cópula
hist(sim_Afix10$ttot, main = "tempo até início da cópula", xlab = "tempo total")

# 2. Scatter Plots

## 2.1 Tempo até amplexo vs Prob. do macho ser escolhido
p1 = ggplot(sim_Afix10, aes(prob, ttot, color=prob)) +
...
## 2.2 Tempo até amplexo vs taxa de foot-flagging
p2 = ggplot(sim_Afix10, aes(footpad, ttot, color=footpad)) +
  geom_point(shape=16, size=.2, show.legend=F, alpha = .5) +
  theme_minimal() +
  scale_color_gradient(low = "darkseagreen1", high = "darkslategrey") +
...
## 2.3 Tempo até amplexo vs DF
p3 = ggplot(sim_Afix10, aes(freqpad, ttot, color=freqpad)) +
...
## 2.4 Tempo até amplexo vs SVL
p4 = ggplot(sim_Afix10, aes(tamapad, ttot, color=tamapad)) +
  geom_point(shape=16, size=.05, show.legend=F, alpha = .5) +
  theme_minimal() +
  scale_color_gradient(low = "darkseagreen1", high = "darkslategrey") +
  xlab("\nTamanho corporal (mm)") +
...
```{r echo=FALSE}
# Histograma de tempo até início da cópula
hist(sim_Avar$ttot, main = "tempo até início da cópula", xlab = "tempo total")
par(mfrow = c(2,2))
plot(sim_Avar$ttot ~ sim_Avar$prob)
plot(sim_Avar$ttot ~ sim_Avar$footpad)
plot(sim_Avar$ttot ~ sim_Avar$freqpad)
plot(sim_Avar$ttot ~ sim_Avar$tamapad)
par(mfrow = c(1,1))
```
