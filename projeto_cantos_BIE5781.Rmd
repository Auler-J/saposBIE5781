...
### Modelos mínimo adequado
Neste cenário, o modelo 1 foi selecionado. Este modelo tem todas as três variáveis e suas interações. 
\

<details>
<summary>Sumário do melhor modelo</summary>
```{r}
...
```{r fig.align= 'center', fig.width=6, fig.height=6}
# Plota 95% CI do melhor modelo
plotConf(glmm10.1)
```

```{r fig.align= 'center', fig.width=8, fig.height=4}
# Plota linhas de tendência de um dos melhores modelos
plot_tendencia(dados = sim_Afix10, modelo = glmm10.1)
```
...
Neste cenário, os modelos 1 e 2 foram considerados igualmente plausíveis pois possuem $\Delta AIC > 2$. Estes modelos diferem apenas na presença da interação tripla <code>tamapad:freqpad:footpad</code>. Por simplicidade, focamos as figuras apresentadas no modelo de menor AIC.
...
Os modelos 1 e 2 são igualmente plausíveis pois $\Delta AIC < 2$. O modelo 1 inclui todas as preditoras <code>tamapad</code>, <code>footpad</code>, <code>freqpad</code> e suas interações duplas e tripla. O modelo 2, por sua vez, não possui a interação tripla. Por ter menor AIC, apresentamos o modelo 1 em detalhes:
...
Seguimos o critério de que um modelo é definitivamente mais plausível se tem $\Delta AIC$ < 2. Em caso de empate, consideramos que os modelos selecionados cuja diferença de AIC é menor do que 2 são igualmente plausíveis. Com exceção da área fixa de 10, nota-se que mais de um modelo foi escolhido por área.
...
cfglmm50.2 = c(summary(glmm10.2)$coefficients[,1], NA)
...
Dentre as curvas de tendências das diferentes áreas, podemos perceber que a curva da simulação com área variável é a que mais se afasta dos dados simulados. Essa discrepância é compreensível, uma vez que existe uma variabilidade muito maior nos dados.
...
Todos os plots de diagnóstico apresentam formato parecido: Os resíduos são muito mais variáveis nos valores ajustados menores e depois se afunilam. Possivelmente, o formato dos resíduos também pode ser explicado pela função de ligação escolhida, que não permitiu flexibilidade ao modelo.

```{r fig.align= 'center', fig.width=6, fig.height=6}
grid.arrange(
  plot(glmm10.1, main = "glmm10.1"),
  plot(glmm50.1, main = "glmm50.1"),
  plot(glmm100.2, main = "glmm100.2"),
  plot(glmmvar.1, main = "glmmvar.1"), nrow = 2)
```
