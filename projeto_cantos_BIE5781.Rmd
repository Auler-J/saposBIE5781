...
nmacho=10
nsim = 1000
posf = data.frame(posx = NA, posy = NA, nfemea = NA)
storepos = list(NA)
...
```{r simulacao}
#set.seed(1)
result = data.frame(nmacho = NA, dcach = NA, foot = NA, freq = NA, tama = NA, footpad = NA, freqpad = NA, tamapad = NA, exprlin = NA, prob = NA, distf = NA, ttot = NA, nsim = NA, femea = NA)
#############################
############ INICIO FOR EXTERNO ###############
###############################################
for(k in 1:nsim){
  ...
  dados = data.frame(nmacho = paste0("m", 1:nmacho), dcach = NA, foot = NA, freq = NA, tama = NA, footpad = NA, freqpad = NA, tamapad = NA, exprlin = NA, prob = NA, distf = NA, ttot = NA, nsim = NA)
  ...
  for(j in 1:nmacho){
    ...
    escolhido = sample(mdisp, 1, prob = dados$prob[dados$nmacho %in% mdisp])
    ...
    dados$femea[dados$nmacho == escolhido] = femeas[j]
    ...
    mindisp[j] = escolhido
    mdisp = dados$nmacho[!dados$nmacho %in% mindisp]
  }
...
hist(result$ttot)
plot(result$ttot ~ result$prob)
plot(result$ttot ~ result$footpad)
plot(result$ttot ~ result$freqpad)
plot(result$ttot ~ result$tamapad)

# Modelos mistos generalizados
glmm1 = glmer(ttot~tamapad*freqpad*footpad + (1|nsim), family = Gamma, data=result)
glmm2 = glmer(ttot~tamapad+freqpad+footpad + tamapad:freqpad + freqpad:footpad +tamapad:footpad
              + (1|nsim), family = Gamma, data=result)
AICtab(glmm1, glmm2)  #melhor glmm e o modelo cheio com interacoes

glmer(ttot~tamapad + (1|nsim), family = Gamma, data=result)
glmer(ttot~footpad + (1|nsim), family = Gamma, data=result)
glmer(ttot~freqpad + (1|nsim), family = Gamma, data=result)

# Modelos generalizados
glm1 = glm(ttot~tamapad*freqpad*footpad, family = Gamma, data=result)
