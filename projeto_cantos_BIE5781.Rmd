...
library(glmmTMB)
glmm10.1t = glmmTMB(ttot~tamapad*freqpad*footpad+(1|nsim), family=Gamma(link="log"), data=sim_Afix10)
...
teste = sim_Avar %>% filter(!row_number() %in% c(1:990000))

glmmvar.1 = glmer(ttot~tamapad*freqpad*footpad + (1|nsim), family=Gamma, data=teste)
