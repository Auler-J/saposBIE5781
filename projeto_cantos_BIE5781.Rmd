...
```{r}
load("data/sim1.Rdata") # mudar a mean freq deve ser 5750
load("data/sim2.Rdata") # mudar a mean freq deve ser 5750
summary(sim1)
summary(sim2)
```
...
p1 = ggplot(sim2, aes(prob, ttot, color=prob)) +
...
p2 = ggplot(sim2, aes(footpad, ttot, color=footpad)) +
  geom_point(shape=16, size=.2, show.legend=F, alpha = .5) +
  theme_minimal() +
  scale_color_gradient(low = "darkseagreen1", high = "darkslategrey") +
...
p3 = ggplot(sim2, aes(freqpad, ttot, color=freqpad)) +
...
p4 = ggplot(sim2, aes(tamapad, ttot, color=tamapad)) +
  geom_point(shape=16, size=.05, show.legend=F, alpha = .5) +
  theme_minimal() +
  scale_color_gradient(low = "darkseagreen1", high = "darkslategrey") +
  xlab("\nTamanho corporal (mm)") +
