...
cfsimulacao = c(0.5, -1.05, 0.8, rep(NA, 5))
...
cftab = data.frame(cfsimulacao, cfglmm10.4, cfglmm10.2, cfglmm10.1, cfglmm50.1, cfglmm100.2, cfglmm100.1, cfglmmvar.2, cfglmmvar.1)
names(cftab) = gsub(pattern = "cfglm", replacement = "", x = names(cftab))
names(cftab)[1] = "sim"
rownames(cftab) = gsub(pattern = "pad", replacement = "", x = rownames(cftab))
cftab = t(cftab)
                      
#voltando coeficientes pra escala dos dados
cftab[-1,] = exp(cftab[-1,])
cftab = round(cftab, 2)
